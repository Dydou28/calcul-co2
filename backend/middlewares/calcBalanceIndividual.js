        ...
        IndiceSource.find({category:'telework'}).exec((err, indices) => {
            if (err) {
             resultat('null')
                ...
                form[distanceHeaterIndex].totalCarbon = ((telechaud*distanceWorkHeater) * distanceWorkDays * 45);
                form[distanceAirIndex].totalCarbon = ((telefroid*distanceWorkAirConditionner) * distanceWorkDays * 45);
        ...
        IndiceSource.find({category:'transport'}).exec(async (err, indices) => {
            if (err) {
             resultat('null')
                ...
                if (label[0] == 'train' || label[0] == 'eurostar' || label[0] == 'thalys'
                || label[0] == 'ter' || label[0] == 'bike' || label[0] == 'bus'
                || label[0] == 'subway' || label[0] == 'scooter' || label[0] == 'car' || label[0] == 'moto') {
                    totalCarbon += row.resp * indiceType;
                    form[form.length-1].totalCarbon = row.resp * indiceType;
                }else if (label[0] == 'plane') { // a modifier
                    totalCarbon += row.resp * indiceType;
                    form[form.length-1].totalCarbon = row.resp * indiceType;
                }else if (label[0] == "carpooling") {
                    totalCarbon += row.resp * indiceType / label[2];
                    form[form.length-1].totalCarbon = row.resp * indiceType / label[2];
                }
                // LE RESULTAT EST RETOURNE EN KILO GRAMME DE CO2 PAR AN
                resultat({form, totalCarbon});
            }
    ...
    return new Promise(resultat => {
        IndiceSource.find({ category: 'numerique' }).exec(async (err, indices) => {
            if (err) {
                ...
                //pour abonnements
                if (row.label == "internetDomicile" || row.label == "internetMobile" || row.label == "internetBureau") {

                    await indices.map((indice) => {
                        indiceType = indice.value;
                        indiceMass = indice.masse;
                    });
                    form[form.length - 1].totalCarbon = row.resp * indiceType * indiceMass;
                ...
                //pour matériel
                    ...
                    await indices.map((indice) => {
                        if (indice.form_name == label[0]+"Utilisation") {
                            ...
                            mass = indice.masse;  
                        } else if (label[1] && indice.form_name == label[1]+"Utilisation") {
                            kiloWatt = indice.value; 
                            mass = indice.masse; 
                        }
                    });

                    utilisationCarbon = nbHeure * (kiloWatt/1000) * mix *(mass/1000) * nbjoursPerAns;

                    if (resp[1] != 0 && resp[2] == "non") { 
                        fabricationCarbon = indiceType * resp[0] * (totalAns / resp[1]);
                        totalCarbon += (fabricationCarbon + utilisationCarbon);
                        form[form.length - 1].totalCarbon = fabricationCarbon + utilisationCarbon;
                    } else if (resp[2] == "oui") {
...
calcAlimentationBalance = async function (form) {
    return new Promise(resultat => {
        IndiceSource.find({category:'alimentation'}).exec(async (err, indices) => {
            if (err) {
             resultat('null')
            } else {
                    var viandeRouge = '';
                ...
                indices.map((indice, i) => {
                    if (indice.form_name === 'viandeRouge' && viandeRougeIndex != null) {
                        totalCarbon += indice.value * indice.masse * viandeRouge;
                        form[viandeRougeIndex].totalCarbon = indice.value * indice.masse * viandeRouge;
                    }else if (indice.form_name === 'viandeBlanche' && viandeBlancheIndex != null) {
                        totalCarbon += indice.value * indice.masse * viandeBlanche;
                        form[viandeBlancheIndex].totalCarbon = indice.value * indice.masse * viandeBlanche;
                    }else if (indice.form_name === 'fruitsSaison' && fruitsSaisonIndex != null) {
                        totalCarbon += indice.value * indice.masse * fruitsSaison;
                        form[fruitsSaisonIndex].totalCarbon = indice.value * indice.masse * fruitsSaison;
                    }else if (indice.form_name === 'fruitsHorsSaison' && fruitsHorsSaisonIndex != null) {
                        totalCarbon += indice.value * indice.masse * fruitsHorsSaison;
                        form[fruitsHorsSaisonIndex].totalCarbon = indice.value * indice.masse * fruitsHorsSaison;
                        ...
                        form[fromageIndex].totalCarbon = indice.value * indice.masse * fromage;
                    }else if (indice.form_name === 'yaourts' && yaourtsIndex != null) {
                        totalCarbon += indice.value * indice.masse * yaourts;
                        form[yaourtsIndex].totalCarbon = indice.value * indice.masse * yaourts;
                    }else if (indice.form_name === 'creme' && cremeIndex != null) {
                        totalCarbon += indice.value * indice.masse * creme;
                        form[cremeIndex].totalCarbon = indice.value * indice.masse * creme;
                    }else if (indice.form_name === 'nbRepasVegan' && nbRepasVeganIndex != null) {
                        totalCarbon += indice.value * indice.masse * nbRepasVegan;
                        form[nbRepasVeganIndex].totalCarbon = indice.value * indice.masse * nbRepasVegan;
                    }else if (indice.form_name === 'poisson' && poissonIndex != null) {
                        totalCarbon += indice.value * indice.masse * poisson;
                        form[poissonIndex].totalCarbon = indice.value * indice.masse * poisson;
                    }else if (indice.form_name === p && nbCafeIndex != nullrepaCafe) {
                        totalCarbon += indice.value * indice.masse * nbCafe;
                        form[nbCafeIndex].totalCarbon = indice.value * indice.masse * nbCafe;
                    }else if (indice.form_name === 'nbChoco' && nbChocoIndex != null) {
                        totalCarbon += indice.value * indice.masse * nbChoco;
                        form[nbChocoIndex].totalCarbon = indice.value * indice.masse * nbChoco;
                    }
                    if (i == l - 1)
                        resultat({form, totalCarbon: totalCarbon * 1000});
                });
            }
          })
             ...
             resultat('null')
                ...
                indices.map((indice, i) => {
                    if (indice.form_name === 'nbLivre' && nbLivreIndex != null) {
                        ...
                        form[nbLivreIndex].totalCarbon =  indice.value * indice.masse * nbLivre;
                    }else if (indice.form_name === 'nbTshirt' && nbTshirtIndex != null) {
                        totalCarbon += indice.value * indice.masse * nbTshirt;
                        form[nbTshirtIndex].totalCarbon =  indice.value * indice.masse * nbTshirt;
                    }else if (indice.form_name === 'nbFourniture' && nbFournitureIndex != null) {
                        totalCarbon += indice.value * indice.masse * nbFourniture;
                        form[nbFournitureIndex].totalCarbon =  indice.value * indice.masse * nbFourniture;
                    }else if (indice.form_name === 'prixConseils' && prixConseilsIndex != null) {
                        totalCarbon += indice.value * indice.masse * prixConseils;
                        form[prixConseilsIndex].totalCarbon =  indice.value * indice.masse * prixConseils;
                    }else if (indice.form_name === 'donsAsso' && donsAssoIndex != null) {
                        totalCarbon += indice.value * indice.masse * donsAsso;
                        form[donsAssoIndex].totalCarbon =  indice.value * indice.masse * donsAsso;
                    }else if (indice.form_name === 'prixComService' && prixComServiceIndex != null) {
                        totalCarbon += indice.value * indice.masse * prixComService;
                        form[prixComServiceIndex].totalCarbon =  indice.value * indice.masse * prixComService;
                    }else if (indice.form_name === 'papier') {
                        totalCarbon += indice.value * indice.masse * papier;
                        form[papierIndex].totalCarbon = indice.value * indice.masse * papier;
                    }
                    if (i == l - 1)
                    resultat({form, totalCarbon});
                });
