...
exports.newBalance = (req, res, next) => {
  ...
  BalanceIndividual.find({ userId: userId }).exec(async (err, balance) => {
    if (err) {
      ...
      await balance.map((b, i) => {
        if (b.terminated === false) balanceIndex = i;
      });
...
exports.getStepBalance = (req, res, next) => {
  ...
  BalanceIndividual.find({ userId: userId }).exec(async (err, balances) => {
    if (err) {
      ...
      let balanceIndex = balances.map((b) => {
        if (b.terminated === false) return b._id;
      });

      if (balanceIndex[balanceIndex.length - 1] && balanceIndex.length > 0) {
        BalanceIndividual.findOne({
          _id: balanceIndex[balanceIndex.length - 1],
        }).exec((err, balance) => {
          res.status(200).json({
            status: true,
...
exports.updateStepBalance = (req, res, next) => {
  ...
  BalanceIndividual.findOne({ _id: balanceId, userId: userId }).exec(
    async (err, balance) => {
      if (err) {
        res.message = "mongoDB error";
        res.status(400).json({
        ...
        let balanceStepIndex = balance.steps
          .map((s, i) => {
            if (s.label === stepName) return s.label;
          })
          .indexOf(stepName);
        let resCalc;
        ...
        if (stepName == "workPlace") {
          ...
          resCalc = await calcBalanceIndividual.calcTransportBalance(
            form,
            balanceStepIndex >= 0
        ...
        if (balanceStepIndex >= 0) {
          updateObject = [
            { _id: balanceId, "steps.label": stepName },
            {
              $set: {
        ...
        BalanceIndividual.updateOne(updateObject[0], updateObject[1]).exec(
          async (err, balance) => {
            if (err) {
              console.log(err);
              res.message = "mongoDB error";
...
exports.getLastBalance = (req, res, next) => {
  ...
  BalanceIndividual.find({ userId: userId })
    .sort([["creationDate", -1]])
    .limit(1)
  ...
  BalanceIndividual.findOne({ _id: balanceId }).exec(async (err, balance) => {
    if (err) {
      ...
      if (balanceStepIndex >= 0) {
        for (
          let i = 0;
          i < balance.steps[balanceStepIndex].questions.length;
          i++
        ) {
        ...
        if (questionIndex >= 0) {
          ...
          BalanceIndividual.updateOne(
            { _id: balanceId, "steps.label": stepName },
            {
              $set: {
                "steps.$.questions": balance.steps[balanceStepIndex].questions,
                "steps.$.totalCarbon":
                  balance.steps[balanceStepIndex].totalCarbon - questionCarbon,
              },
            }
          ).exec(async (err, balance) => {
            if (err) {
