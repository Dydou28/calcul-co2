      ...
      (data: any) => {
        ...
        if (data.status === true && data.balanceStep.length > 0) {
          ...
          let balanceStepIndex = data.balanceStep
            .map((s: any) => {
              if (s && s.label === 'alimentation') return 'alimentation';
              else return null;
            })
            .indexOf('alimentation');
          this.calcpersoForm4.setValue({
            ...
            yaourts:
              data.balanceStep[balanceStepIndex]?.questions[6]?.resp || 0,
            creme: data.balanceStep[balanceStepIndex]?.questions[7]?.resp || 0,
            nbRepasVegan:
              data.balanceStep[balanceStepIndex]?.questions[8]?.resp || 0,
            poisson:
            ...
            nbChoco:
              data.balanceStep[balanceStepIndex]?.questions[12]?.resp || 0,
          });
        }
      },
      (err) => {
