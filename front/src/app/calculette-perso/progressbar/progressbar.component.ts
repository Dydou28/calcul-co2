...
export class ProgressbarComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscriptionName = this.updateService
      .getUpdate()
      .subscribe((message) => {
    ...
    this.calcPersoService.getBalance(this.balanceId).subscribe(
      (data: any) => {
        ...
        data.balance.steps.map((s: any) => {
          if (s.label === 'workPlace') this.workPlace = s.totalCarbon / 1000;
          ...
          else if (s.label === 'achat') this.achat = s.totalCarbon / 1000;
        });
        this.progressTotal =
          this.workPlace +
          this.transport +
